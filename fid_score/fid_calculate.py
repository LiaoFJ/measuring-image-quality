...
def save_fid_stats(real_paths, fake_path, batch_size, device, dims, num_workers=1):
    """Calculates the FID of two paths"""
    if not os.path.exists(real_paths):
        raise RuntimeError('Invalid path: %s' % real_paths)

    if os.path.exists(fake_path):
        raise RuntimeError('Existing output file: %s' % fake_path)

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]

    model = InceptionV3([block_idx]).to(device)

    print(f"Saving statistics for {real_paths}")
...
def main(real_path, fake_path):
    ...
    if args.save_stats:
        save_fid_stats(real_path, fake_path, args.batch_size, device, args.dims, num_workers)
        return
