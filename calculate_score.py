...
def get_argparse():
    ...
    parser.add_argument('real_train_dir', required=True, type=str)
    parser.add_argument('real_test_dir', required=True, type=str)
    parser.add_argument('fake_test_dir', required=True, type=str)
...
if __name__ == '__main__':
    ...
    clip_score = calculate_clip_score(real_dir=args.real_test_dir, fake_dir=args.fake_test_dir)
    clip_aesthetic_score = clip_aesthetic(fake_dir=args.fake_test_dir)
    fid64 = calculate_fid64_score(real_train_dir=args.real_test_dir, fake_dir=args.fake_test_dir)
    lpips_score = calculate_lpips_score(real_dir=args.real_test_dir, fake_dir=args.fake_test_dir)
